...
async function initializeData(id: string) {
  const data = await fetchEntries(id);

  if (data.length) {
    await analyzeLatest(data);
  } else {
    setLoadingAnalysis(false);
  }

  await fetchInsights(id);
}
  ...
  async function analyzeLatest(entryList?: JournalEntry[]) {
  ...
  return (
    <div className="p-10 bg-gray-50 text-gray-800 font-sans">
      ...
      <h1 className="text-blue-600 border-b-4 pb-3 mb-8 text-3xl font-bold">
       AI Journal Analysis
      </h1>

      {/* Journal Entry */}
      <div className="bg-white p-5 rounded-lg shadow-md mb-8">
        <h2 className="text-gray-800 border-b pb-3 mb-5 text-xl">
          Journal Entry
        </h2>
        ...
        <textarea
          ...
          onChange={(e) => setText(e.target.value)}
          placeholder="Write your journal..."
          className="w-full px-4 py-3 border border-gray-300 rounded-lg mb-4 min-h-[120px]"
        />
      ...
      <div className="bg-white p-5 rounded-lg shadow-md">
        ...
        {loadingInsights ? (
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <p><strong>Top Emotion:</strong> {insights.topEmotion}</p>
            <p><strong>Most Used Ambience:</strong> {insights.mostUsedAmbience}</p>
            <p><strong>Keywords:</strong> {insights.recentKeywords?.join(", ")}</p>
          </div>
        )}
      </div>
